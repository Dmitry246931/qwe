  ...
  <body >
      ...
      <table>
          ...
          <tr>
              ...
              <td><?= $mon['cost'] ?></td>
              <td><button class="btn btn-primary" name="Edit" value="<?=$mon['id']?>">Редактировать</button></td>
              <td><button class="btn btn-outline-secondary" name="Delete" value="<?=$mon['id']?>">Удалить</button></td>
          </tr>
          <?php endforeach ?>
...
<p></p>
      <form method="post" enctype="multipart/form-data">
          ...
          <label for="Name_mon">Наименование</label>
          <input type="text" name="Name_mon" id="Name_mon" class="form-control" placeholder="Введите название" value="<?php if(isset($_POST['Name_mon'])){ echo $_POST['Name_mon'];}?>">

          <label for="shar_id">Акции</label>
          <select name="shar_id" class="form-control" id="shar_id">
          <option value="" disabled="" selected="">Выберите акцию</option>
          <?php if(isset($_POST['shar_id']) && !empty($_POST['shar_id'])){
              switch($_POST['shar_id']){
                  case 1:
          ...
          <textarea name="desc" id="desc" maxlength="255" placeholder="Введите описание"><?php if(isset($_POST['desc'])){ echo $_POST['desc'];}?></textarea>

          <button class="btn btn-primary" name="add" value="submit" style="width:10rem;height:2.5rem;margin-top:1rem;">Отправить</button>
