...
<html lang="ru">
  ...
  <body >
      ...
      <div class="container">
      <form class="m-5" action="preset.php" method="post">
        <label class="form-label">Введите текст</label>
        <textarea name="text" class="form-control"><?php if(isset($_POST['text'])){ echo $_POST['text'];}?></textarea>
        <button type="btn btn-primary mt-2">Отправить</button>
      </form>
      ...
      <div class="container">
        ...
        <?php if(!empty($task['task5'])){echo $task['task5'];}?>
        <?php if(!empty($task['task8'])){echo $task['task8'];}?>
        <?php if(!empty($task['task14'])){echo $task['task14'];}?>
        <?php if(!empty($task['task16'])){echo $task['task16'];}?>

        <?php if(!empty($task['all_tasks'])){echo $task['all_tasks'];}?>
